...
def get_norm_layer(norm_type='instance'):
    ...
    """
    if norm_type == 'batch':
        norm_layer = functools.partial(SynchronizedBatchNorm2d, affine=True, track_running_stats=True)
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if 'resnet' in netG and 'blocks' in netG:
        num_resnet_blocks = int(netG.split('_')[1][0])
        if opt.use_rgb == True and opt.dataset == 'depthsynthesis':
            net = ResnetGeneratorWithRGB(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=num_resnet_blocks, opt=opt)
        else:
            net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=num_resnet_blocks, opt=opt)
...
class PatchSampleDiffF(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleDiffF, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            if num_patches > 0:
                if patch_ids is not None:
                    patch_id_i = patch_ids['i'][feat_id]
                    patch_id_j = patch_ids['j'][feat_id]
                else:
                    patch_id_i = torch.randperm(feat_reshape.shape[1], device=feats[0].device)
                    patch_id_j = torch.randperm(feat_reshape.shape[1], device=feats[0].device)
                    patch_id_i = patch_id_i[:int(min(num_patches, patch_id_i.shape[0]))]  # .to(patch_ids.device)
                    patch_id_j = patch_id_j[:int(min(num_patches, patch_id_j.shape[0]))]
                x_sample_i = feat_reshape[:, patch_id_i, :].flatten(0, 1)  # reshape(-1, x.shape[1])
                x_sample_j = feat_reshape[:, patch_id_j, :].flatten(0, 1)
            ...
            if self.use_mlp:
                mlp = getattr(self, 'mlp_%d' % feat_id)
                x_sample = mlp(x_sample)
            return_ids['i'].append(patch_id_i)
            return_ids['j'].append(patch_id_j)
            x_sample = self.l2norm(x_sample)
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(no_antialias):
                ...
                model += [nn.ReflectionPad2d(1),
                          nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=0, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
